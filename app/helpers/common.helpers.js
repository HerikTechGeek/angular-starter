angular
    ...
    .service('commonHelpers', ['$rootScope', '$mdDialog', '$timeout', function($rootScope, $mdDialog, $timeout) {
        ...
        this.confirm = function(objSettings) {
            var _confirm = {
                'title': __translate(objSettings.title),
                'parent': angular.element(document.body),
                'ariaLabel': __translate(objSettings.title),
                'targetEvent': __translate(objSettings.targetEvent),
                'textContent': __translate(objSettings.textContent),
                'ok': __translate(objSettings.ok),
                'cancel': __translate(objSettings.cancel),
            };
            return $mdDialog.show($mdDialog.confirm(_confirm));
        }
        ...
        this.showPrompt = function(objSettings) {
            var confirm = {
                ...
                parent: angular.element(document.body),
                targetEvent: objSettings.targetEvent,
                clickOutsideToClose: objSettings.clickOutsideToClose,
                locals: {
                    title: __translate(objSettings.title),
                    textContent: __translate(objSettings.textContent),
                    placeholder: __translate(objSettings.placeHolder),
                    ariaLabel: __translate(objSettings.ariaLabel),
                    initialValue: objSettings.initialValue,
                    targetEvent: objSettings.targetEvent,
                    entityID: objSettings.entityID,
                    ok: __translate(objSettings.ok),
                    cancel: __translate(objSettings.cancel),
                    postAction: objSettings.postURL
                }
            }
